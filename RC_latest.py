...
def para(bam_files,vcf_informed,from_component,to_component,n_mismatches_allowed=0,gene=None,region=None):
    counts,EH_rep_nos=[],[]
...
def count_and_write(bam_file,component_stretch_counts_list,count_list,components,print_df=False,sort_by_genotype_length=None):
...
try:
    if args.bam_files is not None:
        ...
        for bam_file,component_stretch_counts_list,EH_rep_no in para(bam_files,args.vcf_informed,from_component,to_component,args.n_mismatches,gene,region):
            # print(component_stretch_counts_list)
            if len(component_stretch_counts_list)>0:
                count_and_write(bam_file,component_stretch_counts_list,count_list,components,False,args.sort_by_genotype_length)
    else:
        ...
        # print(component_stretch_counts_list)
        if len(component_stretch_counts_list)>0:
            count_and_write(bam_file,component_stretch_counts_list,count_list,components,True,args.sort_by_genotype_length)
